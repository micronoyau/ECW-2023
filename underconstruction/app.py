...
def subscribe():
    ...
    print(request.form)
    if ("email" in request.form and validate_email(request.form.get("email"), check_smtp = True, check_dns = True) or True):
        print(request.form.get("email"))
        if (any([i in request.form.get("email") for i in BLACKLIST]) or len(request.form.get("email")) > 100) and False:
